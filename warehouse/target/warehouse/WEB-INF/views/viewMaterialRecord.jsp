            ...
            <div class="col-md-12" style="margin-top:10px;">
                ...
                <div class="panel">
                    <div class="panel-heading">
                        <h3 class="panel-title">查看原材料入库记录</h3>
                        <br>
                        <form action="<%=request.getContextPath() %>/controller/goods/viewMaterialRecord.do" method="post">
                    ...
                    <div class="panel-body">
                        ...
                        <ul class="pagination" style="margin-left:15px;">
                            <c:if test="${pagination.currentPage gt 1 }">
                                <li><a href="<%=request.getContextPath() %>/controller/goods/viewMaterialRecord.do?startTime=${startTime}&endTime=${endTime}&materialId=${materialId}&num=${pagination.currentPage-1}">&laquo;</a></li>
                            </c:if>
                                ...
                                <c:when test="${end gt start }">
                                    <c:forEach var="i" begin="${start}" end="${end}">
                                        <c:if test="${i >0 && i<= pageNum}">
                                            ...
                                            <c:if test="${i != pagination.currentPage}"  >
                                                <li><a href="<%=request.getContextPath() %>/controller/goods/viewMaterialRecord.do?startTime=${startTime}&endTime=${endTime}&materialId=${materialId}&num=${i}">${i}</a></li>
                                            </c:if>
                                        </c:if>
                                    </c:forEach>
                                </c:when>
                            ...
                            <c:if test="${pagination.currentPage lt pagination.total/pagination.pageSize }">
                                <li><a href="<%=request.getContextPath() %>/controller/goods/viewMaterialRecord.do?startTime=${startTime}&endTime=${endTime}&materialId=${materialId}&num=${pagination.currentPage+1}">&raquo;</a></li>
                            </c:if>
